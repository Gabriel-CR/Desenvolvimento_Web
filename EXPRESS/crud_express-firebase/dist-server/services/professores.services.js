...
var ProfessorService = /*#__PURE__*/function () {
  ...
  _createClass(ProfessorService, null, [{
    ...
    value: function list(request, response) {
      (0, _firestore.getDocs)((0, _firestore.collection)(_firebaseConnection["default"], "professores")).then(function (professor) {
        var professores = [];
        professor.forEarh(function (prof) {
          var _id = prof.id;
          var _prof$data = prof.data(),
            nome = _prof$data.nome,
            curso = _prof$data.curso,
            titulacao = _prof$data.titulacao,
            ai = _prof$data.ai;
          professores.push({
            _id: _id,
            nome: nome,
